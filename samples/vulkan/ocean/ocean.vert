...
void main()
{
	vec3 ocean_vert = in_position + texture(u_displacement_map, in_tex_coords).rgb * (view.tex_size/view.ocean_size);
	world_coord = ocean_vert;
	gl_Position = view.view_proj_mat * vec4(ocean_vert, 1.0);
	frag_color = vec4(1.0);
